		...
		import('@dbml/core')
			.then((dbmlCore: any) => {
				...
				try {
					...
					const normalized = parsedSchema.normalize ? parsedSchema.normalize() : parsedSchema
	...
	return (
		<div className="flex items-center gap-1 relative">
			...
			<ToolbarButton
				...
				title="Toggle field types"
				active={showFieldTypes}
			>
					...
					<div className="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 z-50 py-1">
						...
						<div className="px-3 py-1">
							<select
								value={sqlDialect}
								onChange={(e) =>
									setSqlDialect(e.target.value as 'postgres' | 'mysql' | 'mssql')
								}
			...
			{showHelp && <div className="fixed inset-0 z-40" onClick={() => setShowHelp(false)} />}
