...
const TYPES = new Set([
  ...
  'uuid', 'json', 'jsonb', 'blob', 'binary',
  'money', 'serial', 'bigserial',
])
// Field options inside []
const FIELD_OPTIONS = new Set([
  'pk', 'primary', 'key', 'not', 'null', 'unique', 'default',
  'increment', 'note', 'ref', 'update', 'delete', 'cascade',
  'restrict', 'no', 'action', 'set',
])
...
const dbmlLanguage = StreamLanguage.define<DBMLState>({
  ...
  token(stream, state) {
    ...
    if (stream.peek() === "'" || stream.peek() === '"') {
      ...
      let ch
      while ((ch = stream.next()) != null) {
        if (ch === state.stringChar) { state.inString = false; break }
        if (ch === '\\') stream.next()
      }
      return 'string'
    }
    ...
    // Track bracket state
    if (stream.peek() === '[') { state.inBracket = true; stream.next(); return 'bracket' }
    if (stream.peek() === ']') { state.inBracket = false; stream.next(); return 'bracket' }
    if (stream.peek() === '{') { stream.next(); return 'bracket' }
    if (stream.peek() === '}') { stream.next(); return 'bracket' }
...
export function dbmlExtensions(dark: boolean): Extension[] {
  return [
    dbmlLanguage,
    syntaxHighlighting(dark ? dbmlHighlightDark : dbmlHighlightLight),
  ]
}
