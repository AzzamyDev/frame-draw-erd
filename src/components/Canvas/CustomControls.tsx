...
export function CustomControls({ darkMode, locked, onToggleLock }: Props) {
	...
	const base = [
		...
		darkMode ? 'hover:bg-white/10 active:bg-white/15' : 'hover:bg-black/8 active:bg-black/12',
	].join(' ')

	const divider = darkMode ? 'h-px bg-white/10' : 'h-px bg-black/8'
	...
	return (
		<Panel position="bottom-left" style={{ margin: 16 }}>
			<div className={wrap}>
				{/* Zoom In */}
				<button className={base} title="Zoom in" onClick={() => zoomIn({ duration: 200 })}>
				...
				{/* Zoom Out */}
				<button className={base} title="Zoom out" onClick={() => zoomOut({ duration: 200 })}>
