...
const PALETTE = [
  ...
  '#0891b2', '#db2777', '#4b5563', '#4338ca', '#0d9488',
]

function FieldBadge({ label, color }: { label: string; color: string }) {
  return (
    <span className={`inline-flex items-center px-1 py-0.5 rounded text-[9px] font-medium leading-none ${color}`}>
      {label}
    </span>
  )
}
  ...
  return createPortal(
    ...
    >
      ...
      <div className="flex flex-col gap-1.5">
        {hasNote && (
          <div>
            <span className="text-gray-400 font-medium uppercase tracking-wide text-[9px]">Note</span>
            <p className="text-gray-100 mt-0.5 leading-relaxed break-words">{field.note}</p>
          </div>
        )}
        {hasDefault && (
          <div>
            <span className="text-gray-400 font-medium uppercase tracking-wide text-[9px]">Default</span>
            <p className="text-blue-300 mt-0.5 font-mono">{field.default}</p>
          </div>
        )}
        {hasEnum && (
          <div>
            <span className="text-gray-400 font-medium uppercase tracking-wide text-[9px]">Enum values</span>
            <div className="flex flex-wrap gap-1 mt-0.5">
              {field.enumValues!.map(v => (
                <span key={v} className="bg-indigo-800/60 text-indigo-200 px-1.5 py-0.5 rounded text-[10px] font-mono">{v}</span>
              ))}
            </div>
          </div>
        )}
...
function FieldRow({ field, tableName, index, onDoubleClick }: {
  field: ParsedField
  tableName: string
  index: number
  onDoubleClick: () => void
  ...
  return (
    ...
    >
      <Handle type="target" position={Position.Left} id={leftId}
        style={{
          position: 'absolute', top: '50%', left: -8, transform: 'translateY(-50%)',
          width: 14, height: 14, borderRadius: '50%', zIndex: 20, cursor: 'crosshair',
      ...
      <span className="text-gray-400 dark:text-gray-500 text-[10px] shrink-0 ml-2 font-mono">{field.type}</span>

      <div className="flex items-center gap-0.5 ml-1 shrink-0">
        {field.pk       && <FieldBadge label="PK"   color="bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-400" />}
        {field.notNull && !field.pk && <FieldBadge label="NN" color="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400" />}
        {field.unique   && <FieldBadge label="UQ"   color="bg-teal-100 dark:bg-teal-900/50 text-teal-700 dark:text-teal-400" />}
        {field.increment && <FieldBadge label="AI"  color="bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-400" />}
        {field.enumValues && <FieldBadge label="ENUM" color="bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-400" />}
        {hasTooltip && (
          <Info size={10} className={`ml-0.5 transition-colors ${hovered ? 'text-blue-400' : 'text-gray-300 dark:text-gray-600'}`} />
        )}
      </div>

      <Handle type="source" position={Position.Right} id={rightId}
        style={{
...
export function TableNode({ data, selected }: NodeProps) {
  ...
  const handleToggle = (e: React.MouseEvent) => {
    ...
    const PICKER_W = 140; const PICKER_H = 80; const left = Math.max(4, Math.min(rect.right - PICKER_W, window.innerWidth - PICKER_W - 4)); const top = rect.bottom + 4 + PICKER_H > window.innerHeight ? rect.top - PICKER_H - 4 : rect.bottom + 4; setPickerPos({ top, left })
    setPickerOpen(v => !v)
  }
    ...
    <>
            ...
            {PALETTE.map(c => (
              <button
                ...
                className={`rounded-full border-2 transition-transform hover:scale-110 ${
                  headerColor === c ? 'border-white scale-110 shadow-lg' : 'border-transparent'
                }`}
                onClick={() => { setNodeColor(table.name, c); setPickerOpen(false) }}
              />
            ))}
          </div>
        </div>,
      ...
      >
        ...
        >
          ...
          <span className="text-white font-bold text-sm truncate flex-1 tracking-tight">{table.name}</span>
          {table.note && (
            <span title={table.note} className="text-white/70 cursor-help shrink-0"><Info size={13} /></span>
          )}
          <button
            ref={buttonRef}
        ...
        <div className="divide-y divide-gray-100 dark:divide-gray-800">
          {table.fields.map((field, idx) => (
            <FieldRow
              key={field.name} field={field} tableName={table.name} index={idx}
              onDoubleClick={() => setFocusTarget({ table: table.name, field: field.name })}
            />
          ))}
          {table.fields.length === 0 && (
            <div className="px-3 py-3 text-xs text-gray-400 italic text-center">No fields</div>
          )}
        </div>
      </div>
    </>
