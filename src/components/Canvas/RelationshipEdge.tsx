...
function buildPath(raw: Waypoint[], radius = 10): string {
	...
	for (let i = 1; i < pts.length - 1; i++) {
		...
		const p1 = {
			x: curr.x + (prev.x - curr.x) * (r1 / d1),
			y: curr.y + (prev.y - curr.y) * (r1 / d1),
		}
		const p2 = {
...
export function RelationshipEdge({
				...
				setEdges((edges) =>
					edges.map((edge) => {
						...
						return {
							...
							data: {
								...(edge.data as object),
								waypoints: [...wps.slice(0, safeWi), startPos, ...wps.slice(safeWi)],
							},
						}
					}),
				)
				...
				setEdges((edges) =>
					edges.map((edge) => {
						...
						return {
							...
							data: {
								...(edge.data as object),
								waypoints: wps.map((wp, i) => (i === wpIdx ? pos : wp)),
							},
						}
					}),
				)
	...
	// ── Waypoint drag ────────────────────────────────────────────────────────────
	const onWpPointerMove = useCallback(
			...
			setEdges((edges) =>
				edges.map((edge) => {
					...
					return {
						...
						data: {
							...(edge.data as object),
							waypoints: wps.map((wp, i) => (i === idx ? pos : wp)),
						},
					}
				}),
			)
	...
	const onWpDoubleClick = useCallback(
		(idx: number, e: React.MouseEvent) => {
			...
			setEdges((edges) =>
				edges.map((edge) => {
					...
					return {
						...edge,
						data: { ...(edge.data as object), waypoints: wps.filter((_, i) => i !== idx) },
					}
				}),
			)
		},
						...
						<div className="relative flex flex-col items-center gap-1.5">
							...
							{pickerOpen && (
								...
								>
									...
									<p className="text-[10px] text-gray-400 text-center mb-2 font-medium tracking-wide">
										Edge Color
									</p>
