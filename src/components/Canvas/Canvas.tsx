...
export function Canvas() {
  ...
  const visibleNodes = showEnums
    ? nodes
    : nodes.filter(n => n.type !== 'enumNode')
  ...
  const onReconnect = useCallback(
    (oldEdge: Edge, newConnection: Connection) => {
      ...
      reconnectRef(oldSrc, oldTgt, source.tableName, source.fieldName, target.tableName, target.fieldName)
    },
    [reconnectRef]
  )
      ...
      >
        ...
        <Panel position="bottom-left" style={{ margin: 16 }}>
          <div className={[
            ...
            {/* Select / Pan mode */}
            <button
              title={selectMode ? 'Switch to pan mode (drag to pan)' : 'Switch to select mode (drag to select)'}
