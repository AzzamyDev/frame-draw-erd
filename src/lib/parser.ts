...
export function parseDBML(code: string): ParseResult {
	...
	const tables: ParsedTable[] = (schema.tables || []).map((table: any) => {
		...
		const fields: ParsedField[] = (table.fields || []).map((field: any) => {
			...
			return {
				...
				note: typeof field.note === 'string' ? field.note : (field.note?.value ?? undefined),
				isFk,
	...
	;(parsedAny.refs || schema.refs || []).forEach((ref: any, idx: number) => {
		...
		const sourceField = Array.isArray(ep1.fieldNames)
			? ep1.fieldNames[0]
			: ep1.fieldName || ep1.field?.name
		const targetField = Array.isArray(ep2.fieldNames)
			? ep2.fieldNames[0]
			: ep2.fieldName || ep2.field?.name
