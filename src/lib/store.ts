...
interface DBStore {
  ...
  reconnectRef: (oldSrcHandle: string, oldTgtHandle: string, newSrcTable: string, newSrcField: string, newTgtTable: string, newTgtField: string) => void
...
export const useStore = create<DBStore>((set, get) => ({
    ...
    try {
      ...
      const newEdges = result.edges.map(newEdge => {
        const existing = currentEdges.find(
          e => e.sourceHandle === newEdge.sourceHandle && e.targetHandle === newEdge.targetHandle
        )
  ...
  reconnectRef: (oldSrcHandle, oldTgtHandle, newSrcTable, newSrcField, newTgtTable, newTgtField) => {
    const parseHandle = (handle: string): string | null => {
      const isRight = handle.endsWith('-right')
      const isLeft = handle.endsWith('-left')
      if (!isRight && !isLeft) return null
